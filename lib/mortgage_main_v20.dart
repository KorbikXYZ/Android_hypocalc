...
class _BarTabState extends State<_BarTab> {
  ...
  Widget build(BuildContext ctx) {
    ...
    return Column(children: [
      ...
      // ── Selected month detail (animated slide-in) ────────────
      ClipRect(
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 200),
          height: _sel != null ? 76 : 0,
          child: _sel != null
            ? _MonthDetail(
                lc: widget.lc,
                entry: sched[_sel!],
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 6),
          Row(children: [
            _det(kG, s('prin'), fmtC(entry.principalPmt)),
            const SizedBox(width: 14),
            _det(kO, s('int'), fmtC(entry.interestPmt)),
            if (fixed > 0.001) ...[
              const SizedBox(width: 14),
              _det(kFixed, s('fixed'), fmtC(fixed)),
            ],
            const SizedBox(width: 14),
            _det(kSub, s('bal'), fmtC(entry.balance)),
          ]),
        ])),
        IconButton(
          onPressed: onClose,
